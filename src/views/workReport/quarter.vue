<template>
  <div class="bgfff out_wrap">
    ...
    <el-dialog :title="`${mode === 1 ? '新增' : '编辑'}季报`" :visible.sync="addVisible" width="690px" center>
      <el-form ref="addForm" :model="addForm" label-suffix=":" :rules="rules" :disabled="mode === 3" label-width="180px">
...
export default {
  ...
  methods: {
    async query() {
      ...
      const res = await getQuarterReportList({ ...query, ...this.pages })
      if (res.code === 0) {
        this.pages.total = res.data.quarterReportList.total
        this.tableList = res.data.quarterReportList.rows
      }
    },
    ...
    async edit(row, type) {
      ...
      // 通过ids获取文件
      this.addForm.fileIds = await this.getFilesFn(row.fileIds) // 季报文件id（多个时用逗号隔开，如：1,2,3）
    },
    del(row) {
      this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
        ...
        const res = await deteleQuarterReportById({ id: row.id })
        if (res.code === 0) {
          this.$message.success('删除季报成功!')
          this.query()
        }
      }).catch(() => {
        this.$message({
    ...
    handleSubmit(formName) {
      this.$refs[formName].validate(async(valid) => {
        if (valid) {
          if (this.mode === 1) {
            const res = await saveQuarterReport({
              ...this.addForm,
              fileIds: this.addForm.fileIds.map(item => item.id).join(',') || '' // 季报
            })
            if (res.code === 0) {
              this.addVisible = false
              this.$message.success('新增季报成功!')
              this.query()
            }
          } else {
            const res = await updateQuarterReport({
              ...this.addForm,
              fileIds: this.addForm.fileIds.map(item => item.id).join(',') || '' // 季报
            })
            if (res.code === 0) {
              this.addVisible = false
              this.$message.success('编辑季报成功!')
              this.query()
            }
          }
        }
